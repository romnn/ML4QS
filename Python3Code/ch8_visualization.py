...
for t in range(1, len(time)):
    pred_mood.append(
        ...
        + x[-1]
        * (
            gamma_1 * (1 - pred_mood[-1]) * max((pred_al[-1] - pred_mood[-1]), 0)
            + gamma_2 * pred_mood[-1] * (min((pred_al[-1] - pred_mood[-1]), 0))
        )
        * delta_t
    )
    pred_al.append(
        ...
        + gamma_3
        * (
            pred_al[-1] * min(math.sin(((t - (gamma_4 * math.pi)) / gamma_5)), 0)
            + (1 - pred_al[-1])
            * max(math.sin(((t - (gamma_4 * math.pi)) / gamma_5)), 0)
        )
    )
    x.append(x[-1])
...
random.seed(0)
random_time_series = pd.DataFrame(
    np.random.normal(0.1, 2, 5200), index=range(0, 5200), columns=["value"]
)
...
plot.ylabel("value")
plot.legend(
    ["$original$ $series$", "$\\alpha=0.2$", "$\\alpha=0.05$"], fontsize="small"
)
plot.show()
...
random_time_series = pd.DataFrame(
    np.random.normal(0, 1, 5200), index=range(0, 5200), columns=["value"]
)
random_time_series["value"] = random_time_series["value"] + 5
random_time_series.iloc[1000:3999, random_time_series.columns.get_loc("value")] = (
    random_time_series.iloc[1000:3999, random_time_series.columns.get_loc("value")] + 20
)
linear_deduction = np.arange(0, -20, -(float(20) / 2000))
random_time_series.iloc[2000:3999, random_time_series.columns.get_loc("value")] = (
    random_time_series.iloc[2000:3999, random_time_series.columns.get_loc("value")]
    + linear_deduction[0:1999]
)

plot.plot(random_time_series.index, random_time_series["value"], "b-")
plot.plot(
    random_time_series.index, random_time_series["value"].ewm(alpha=0.05).mean(), "r:"
)
...
plot.ylabel("value")
plot.legend(
    ["$original$ $series$", "$trend$ $using $ $\\alpha=0.05$", "$detrended$"],
    fontsize="small",
)
plot.show()
...
dataset["radius"] = (
    dataset["filtered_acc_x"].pow(2)
    + dataset["filtered_acc_y"].pow(2)
    + dataset["filtered_acc_z"].pow(2)
)
dataset["radius"] = dataset["radius"].pow(0.5)
plot.plot(dataset.index, dataset["radius"], "r-")
plot.legend(
    ["$original$ $series$", "$filtered$", "$||a||(filtered)$"], fontsize="small"
)
plot.show()
...
pacf_x, confint = pacf(temp_ts, nlags=100, alpha=0.05)
...
y2 = conf[:, 1]
plot.fill_between(
    temp_ts.index[400:500], y1, y2, where=y2 >= y1, facecolor="grey", interpolate=True
)
...
xfmt = md.DateFormatter("%H:%M:%S")
df = pd.DataFrame(
    temp_ts[0:500], index=pd.to_datetime(temp_ts.index[0:500]), columns=["x"]
)
...
axarr[0].legend(
    [
        "$mood$",
        "$activity$ $level$",
        "$predicted$ $mood$ $with$ $\gamma_{1}=\gamma_{2}=\gamma_{3}=\gamma_{4}=\gamma_{5}=1$",
        "$predicted$ $mood$ $with$ $\gamma_{1}=\gamma_{2}=\gamma_{3}=\gamma_{4}=\gamma_{5}=1$",
    ],
    loc=4,
    fontsize="small",
...
axarr[1].legend(
    [
        "$mood$",
        "$activity$ $level$",
        "$predicted$ $mood$ $with$ $\gamma_{1}=5, \gamma_{2}=0.75, \gamma_{3}=0.3, \gamma_{4}=\gamma_{5}=1$",
        "$predicted$ $mood$ $with$ $\gamma_{1}=5, \gamma_{2}=0.75, \gamma_{3}=0.3, \gamma_{4}=\gamma_{5}=1$",
    ],
    loc=4,
    fontsize="small",
...
plot.legend(
    ["$pareto$ $front$", "$non-dominated$ $instance$", "$dominated$ $instance$"],
    loc=1,
    fontsize="small",
    numpoints=1,
)
