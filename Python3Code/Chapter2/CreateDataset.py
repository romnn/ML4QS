...
class CreateDataset:
    ...
    def add_numerical_dataset(
        self, file, timestamp_col, value_cols, aggregation="avg", prefix=""
    ):
        ...
        if self.data_table is None:
            self.create_dataset(
                min(dataset[timestamp_col]),
                max(dataset[timestamp_col]),
                value_cols,
                prefix,
            )
        else:
        ...
        for i in range(0, len(self.data_table.index)):
            ...
            for col in value_cols:
                ...
                if len(relevant_rows) > 0:
                    if aggregation == "avg":
                        self.data_table.loc[
                            self.data_table.index[i], str(prefix) + str(col)
                        ] = np.average(relevant_rows[col])
                    else:
    ...
    def add_event_dataset(
        self, file, start_timestamp_col, end_timestamp_col, value_col, aggregation="sum"
    ):
        print(f"Reading data from {file}")
        ...
        if self.data_table is None:
            self.create_dataset(
                min(dataset[start_timestamp_col]),
                max(dataset[end_timestamp_col]),
                event_values,
                value_col,
            )
        ...
        for i in range(0, len(dataset.index)):
            ...
            relevant_rows = self.data_table[
                (
                    start
                    <= (
                        self.data_table.index + timedelta(milliseconds=self.granularity)
                    )
                )
            ...
            if aggregation == "sum":
                self.data_table.loc[
                    relevant_rows.index, str(value_col) + str(value)
                ] += 1
