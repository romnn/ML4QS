...
class NumericalAbstraction:
    ...
    def abstract_numerical(
        self, data_table, cols, window_size, aggregation_function_name
    ):
        ...
        for col in cols:
            ...
            data_table[
                col + "_temp_" + aggregation_function_name + "_ws_" + str(window_size)
            ] = aggregations
...
class CategoricalAbstraction:
    ...
    def determine_pattern_times(self, data_table, pattern, window_size):
        ...
        # If we have a complex pattern (<n> (b) <m> or <n> (c) <m>)
            ...
            time_points_first_part = self.determine_pattern_times(
                data_table, pattern[0], window_size
            )
            time_points_second_part = self.determine_pattern_times(
                data_table, pattern[2], window_size
            )
            ...
            if support >= min_support:
                ...
                data_table.iloc[
                    ...
                    data_table.columns.get_loc(
                        self.pattern_prefix + self.to_string(pattern)
                    ),
                ] = 1
    ...
    def abstract_categorical(
        self, data_table, cols, match, min_support, window_size, max_pattern_size
    ):
        ...
        for i in range(0, len(cols)):
            if match[i] == "exact":
                ...
                relevant_dataset_cols.extend(
                    [name for name in col_names if cols[i] in name]
                )

        # Generate the one patterns first
        potential_1_patterns = [[pattern] for pattern in relevant_dataset_cols]

        new_data_table, one_patterns = self.select_k_patterns(
            data_table, potential_1_patterns, min_support, window_size
        )
        ...
        while (k < max_pattern_size) & (len(k_patterns) > 0):
            ...
            new_data_table, selected_new_k_patterns = self.select_k_patterns(
                new_data_table, potential_k_patterns, min_support, window_size
            )
