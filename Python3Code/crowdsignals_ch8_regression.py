...
time_features = [
    name for name in dataset.columns if ("temp_" in name and not "hr_watch" in name)
]
freq_features = [
...
features_after_chapter_4 = list(
    set().union(basic_features, pca_features, time_features, freq_features)
)
features_after_chapter_5 = list(
    set().union(
        basic_features, pca_features, time_features, freq_features, cluster_features
    )
)

selected_features = [
    "temp_pattern_labelOnTable",
    "labelOnTable",
    "temp_pattern_labelOnTable(b)labelOnTable",
    "cluster",
    "pca_1_temp_mean_ws_120",
...
possible_feature_sets = [
    ...
    features_after_chapter_4,
    features_after_chapter_5,
    selected_features,
]
feature_names = [
    "initial set",
...
for i in range(0, len(possible_feature_sets)):
    ...
    for repeat in range(0, repeats):
        ...
        regr_train_y, regr_test_y = learner.recurrent_neural_network(
            selected_train_X, train_y, selected_test_X, test_y, gridsearch=True
        )
    ...
    scores_with_sd = [
        (
            overall_performance_tr_res,
            overall_performance_tr_res_std,
            overall_performance_te_res,
            overall_performance_te_res_std,
        ),
        (
            overall_performance_tr_rnn,
            overall_performance_tr_rnn_std,
            overall_performance_te_rnn,
            overall_performance_te_rnn_std,
        ),
        (
            overall_performance_tr_ts,
            overall_performance_tr_ts_std,
            overall_performance_te_ts,
            overall_performance_te_ts_std,
        ),
    ]
    util.print_table_row_performances_regression(
        feature_names[i],
        len(selected_train_X.index),
...
DataViz.plot_performances_regression(
    ["Reservoir", "RNN", "Time series"], feature_names, scores_over_all_algs
)
...
DataViz.plot_numerical_prediction_versus_real(
    train_X.index,
    train_y,
    regr_train_y["hr_watch_rate"],
    test_X.index,
    test_y,
    regr_test_y["hr_watch_rate"],
    "heart rate",
)
regr_train_y, regr_test_y = learner.recurrent_neural_network(
    train_X[basic_features], train_y, test_X[basic_features], test_y, gridsearch=True
)
DataViz.plot_numerical_prediction_versus_real(
    train_X.index,
    train_y,
...
train_X, test_X, train_y, test_y = prepare.split_single_dataset_regression(
    ...
    ["acc_phone_x", "acc_phone_y"],
    0.9,
    filter=False,
    temporal=True,
)

output_sets = learner.dynamical_systems_model_nsga_2(
    train_X,
    train_y,
    test_X,
    test_y,
...
DataViz.plot_numerical_prediction_versus_real_dynsys_mo(
    train_X.index, train_y, test_X.index, test_y, output_sets, 0, "acc_phone_x"
)

regr_train_y, regr_test_y = learner.dynamical_systems_model_ga(
    train_X,
    train_y,
    test_X,
    test_y,
...
DataViz.plot_numerical_prediction_versus_real(
    train_X.index,
    train_y["acc_phone_x"],
    regr_train_y["acc_phone_x"],
    test_X.index,
    test_y["acc_phone_x"],
    regr_test_y["acc_phone_x"],
    "acc_phone_x",
)
