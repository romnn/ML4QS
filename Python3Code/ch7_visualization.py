...
plot.plot([0, 1], [1, 0], "k:")
plot.legend(
    ["$class$ $1$ $(active)$", "$class$ $2$ $(inactive)$", "$decision$ $boundary$"],
    loc=4,
    fontsize="small",
    numpoints=1,
)
...
df1 = pd.DataFrame(
    np.vstack(
        [
            np.hstack(
                [
                    np.random.normal(50, 5, size=(50, 1)),
                    np.random.normal(80, 5, size=(50, 1)),
                ]
            ),
            np.hstack(
                [
                    np.random.normal(50, 5, size=(50, 1)),
                    np.random.normal(20, 5, size=(50, 1)),
                ]
            ),
        ]
    ),
...
ax.annotate(
    ...
    xytext=(0.3, 0.1),
    arrowprops=dict(
        facecolor="black", shrink=0.05, width=1, headwidth=3.5, headlength=3
    ),
    fontsize=15,
)
ax.annotate(
...
ax.annotate(
    "", (x[2], y_1[2]), (x[4] - 0.02, y_2[4] + 0.02), arrowprops={"arrowstyle": "<->"}
)
ax.text(
    0.25,
    0.6,
...
fig = plot.figure(figsize=plot.figaspect(2.0))
ax = fig.add_subplot(1, 2, 1)

ax.plot(x_1, y_1, "ro")
ax.plot(x_2, y_2, "bo")
ax.legend(
    ["$class$ $1$ $(other$ $activity)$", "$class$ $2$ $(walking)$"],
    loc=2,
    fontsize="small",
    numpoints=1,
)
...
z_1 = np.power(
    ...
    -(
        sklearn.metrics.pairwise.euclidean_distances(
            X=df1, Y=np.array([0.5, 0.5]).reshape(1, -1)
        )
        / 2
        * math.pow(sigma, 2)
    ),
...
z_2 = np.power(
    ...
    -(
        sklearn.metrics.pairwise.euclidean_distances(
            X=df2, Y=np.array([0.5, 0.5]).reshape(1, -1)
        )
        / 2
        * math.pow(sigma, 2)
    ),
...
distances_df_full = sklearn.metrics.pairwise.euclidean_distances(
    X=df_full, Y=np.array([0.51, 0.51]).reshape(1, -1)
).flatten()
