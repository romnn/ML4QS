...
features, correlations = fs.pearson_selection(
    10, train_X[features_after_chapter_5], train_y
)
...
for i in range(0, len(possible_feature_sets)):
    ...
    for repeat in range(0, REPEATS):
        ...
        regr_train_y, regr_test_y = learner.random_forest(
            selected_train_X, train_y, selected_test_X, gridsearch=True
        )
        mean_tr, std_tr = eval.mean_squared_error_with_std(train_y, regr_train_y)
        mean_te, std_te = eval.mean_squared_error_with_std(test_y, regr_test_y)
    ...
    regr_train_y, regr_test_y = learner.support_vector_regression_without_kernel(
    ...
    regr_train_y, regr_test_y = learner.k_nearest_neighbor(
        selected_train_X, train_y, selected_test_X, gridsearch=True
    )
    mean_tr, std_tr = eval.mean_squared_error_with_std(train_y, regr_train_y)
    mean_te, std_te = eval.mean_squared_error_with_std(test_y, regr_test_y)
    ...
    scores_with_sd = [
        (
            overall_performance_tr_nn,
            overall_performance_tr_nn_std,
            overall_performance_te_nn,
            overall_performance_te_nn_std,
        ),
        (
            overall_performance_tr_rf,
            overall_performance_tr_rf_std,
            overall_performance_te_rf,
            overall_performance_te_rf_std,
        ),
        (
            performance_tr_svm,
            performance_tr_svm_std,
            performance_te_svm,
            performance_te_svm_std,
        ),
        (
            performance_tr_knn,
            performance_tr_knn_std,
            performance_te_knn,
            performance_te_knn_std,
        ),
        (
            performance_tr_dt,
            performance_tr_dt_std,
            performance_te_dt,
            performance_te_dt_std,
        ),
    ]
...
DataViz.plot_performances_regression(
    ["NN", "RF", "SVM", "KNN", "DT"], feature_names, scores_over_all_algs
)
