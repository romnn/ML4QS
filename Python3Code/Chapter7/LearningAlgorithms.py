...
class RegressionAlgorithms:
    ...
    def feedforward_neural_network(
        ...
        if gridsearch:
            ...
            nn = GridSearchCV(
                MLPRegressor(), tuned_parameters, cv=5, scoring="neg_mean_squared_error"
            )
        else:
            # Create the model
            nn = MLPRegressor(
    ...
    def support_vector_regression_with_kernel(
        self,
        train_X,
        train_y,
        test_X,
        ...
        if gridsearch:
            ...
            svr = GridSearchCV(
                SVR(), tuned_parameters, cv=5, scoring="neg_mean_squared_error"
            )
        else:
            # Create the model
    ...
    # test and training set. It returns the predictions for the training and test set.
    def support_vector_regression_without_kernel(
        self,
        train_X,
        train_y,
        test_X,
        ...
        if gridsearch:
            ...
            svr = GridSearchCV(
                LinearSVR(), tuned_parameters, cv=5, scoring="neg_mean_squared_error"
            )
        else:
            # Create the model
    ...
    def decision_tree(
        ...
        criterion="mse",
        print_model_details=False,
        export_tree_path="./figures/crowdsignals_ch7_regression/",
        export_tree_name="tree.dot",
        gridsearch=True,
    ):
        # Create the model
        if gridsearch:
            tuned_parameters = [
                {"min_samples_leaf": [2, 10, 50, 100, 200], "criterion": ["mse"]}
            ]
            dtree = GridSearchCV(
