...
class PrepareDatasetForLearning:
        ...
        for i in range(0, len(dataset.index)):
            ...
            if sum_values[i] == 1:
                dataset.iloc[i, dataset.columns.get_loc(self.class_col)] = (
                    dataset[labels].iloc[i].idxmax(axis=1)
                )
    ...
    def split_single_dataset_regression_by_time(
        self, dataset, target, start_training, end_training, end_test
    ):
    ...
    # We return a training set, the labels of the training set, and the same for a test set. We can set the random seed
    # to make the split reproducible.
    def split_single_dataset_regression(
        self,
        dataset,
        targets,
        training_frac,
    ...
    # all data of training_frac users and test data for the remaining users. If the later, it return the training_frac
    # data of each user as a training set, and 1-training_frac data as a test set.
    def split_multiple_datasets_regression(
        self,
        datasets,
        targets,
        training_frac,
        ...
        (
            ...
            training_set_y,
            test_set_y,
        ) = self.split_multiple_datasets_classification(
            datasets,
            targets,
            "exact",
            training_frac,
