...
def main():
    ...
    if FLAGS.mode == "imputation":
        ...
        DataViz.plot_dataset(
            kalman_dataset,
            ["acc_phone_x", "acc_phone_x_kalman"],
            ["exact", "exact"],
            ["line", "line"],
        )
        ...
        new_dataset = LowPass.low_pass_filter(
            copy.deepcopy(dataset), "acc_phone_x", fs, cutoff, order=10
        )
        DataViz.plot_dataset(
            new_dataset.iloc[
                int(0.4 * len(new_dataset.index)) : int(0.43 * len(new_dataset.index)),
                :,
            ],
            ["acc_phone_x", "acc_phone_x_lowpass"],
            ["exact", "exact"],
            ["line", "line"],
        )
        ...
        DataViz.plot_xy(
            x=[range(1, len(selected_predictor_cols) + 1)],
            y=[pc_values],
        ...
        DataViz.plot_dataset(
            dataset, ["pca_", "label"], ["like", "like"], ["line", "points"]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["lowpass", "imputation", "PCA", "final"],
    )
