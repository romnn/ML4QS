...
plt.legend(
    ["$measurements$", "$manhattan$ $distance$", "$euclidean$ $distance$"],
    loc=4,
    fontsize="small",
)
plt.xlabel("$X_{1}$")
...
axarr[0, 0].text(
    0, 0.65, "1", fontsize=12, bbox={"facecolor": "grey", "alpha": 0.5, "pad": 10}
)
axarr[0, 0].text(
    0.2,
    0.8,
    "$x_{1,qs_{i}}^{1},\dots,x_{N_{qs_{i}},qs_{i}}^{1}$",
...
axarr[0, 0].text(
    0.2,
    0.2,
    "$x_{1,qs_{i}}^{p},\dots,x_{N_{qs_{i}},qs_{i}}^{p}$",
    verticalalignment="bottom",
    horizontalalignment="left",
    color="black",
    fontsize=10,
...
axarr[0, 0].arrow(
    0.75, 0.65, 0.15, 0, head_width=0.05, head_length=0.05, fc="k", ec="k"
)
...
axarr[2, 0].text(
    0, 0.65, "2", fontsize=12, bbox={"facecolor": "grey", "alpha": 0.5, "pad": 10}
)
axarr[2, 0].text(
    0.2,
    0.8,
...
axarr[2, 2].text(
    0.3,
    0.55,
    "$cluster$ $on$ $distribution$ $parameters$",
    verticalalignment="bottom",
    horizontalalignment="left",
    color="black",
    fontsize=12,
...
axarr[4, 0].text(
    0, 0.65, "3", fontsize=12, bbox={"facecolor": "grey", "alpha": 0.5, "pad": 10}
)
axarr[4, 0].text(
    0.2,
    0.8,
...
axarr[5, 0].text(
    0.2,
    0.8,
    "$x_{1,qs_{j}}^{1},\dots,x_{N_{qs_{j}},qs_{j}}^{1}$",
    verticalalignment="bottom",
    horizontalalignment="left",
    color="black",
    fontsize=10,
...
axarr[5, 0].text(
    0.2,
    0.2,
    "$x_{1,qs_{j}}^{p},\dots,x_{N_{qs_{j}},qs_{j}}^{p}$",
    verticalalignment="bottom",
    horizontalalignment="left",
    color="black",
    fontsize=10,
...
axarr[5, 2].text(
    0.3,
    0.55,
    "$cluster$ $on$ $p$ $values$ $between$ $i$ $and$ $j$",
    verticalalignment="bottom",
    horizontalalignment="left",
    color="black",
    fontsize=12,
...
numbers = np.vstack(
    [np.random.randint(10, 20, size=(10, 2)), np.random.randint(70, 90, size=(10, 2))]
)
...
axarr[0, 0].legend(
    ["$centers$", "$data$ $points$"], loc=4, fontsize="small", numpoints=1
)
axarr[0, 0].set_xlim([0, 1])
axarr[0, 0].set_ylim([0, 1])
...
cluster = np.argmin(
    sklearn.metrics.pairwise.euclidean_distances(X=df, Y=centers), axis=1
)
df["cluster"] = cluster
...
cluster = np.argmin(
    sklearn.metrics.pairwise.euclidean_distances(X=df[["X", "Y"]], Y=centers), axis=1
)
df["cluster"] = cluster
...
numbers = np.vstack(
    [
        np.random.randint(0, 20, size=(20, 2)),
