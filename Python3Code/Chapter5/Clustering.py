...
class NonHierarchicalClustering:
    ...
    def gowers_similarity(self, X, Y=None, Y_norm_squared=None, squared=False):
        ...
        for x_row in range(0, X.shape[0]):
            ...
            for y_row in range(0, Y.shape[0]):
                ...
                distances[x_row, y_row] = DM.gowers_similarity(
                    data_row1, data_row2, self.p
                )
    ...
    def k_means_over_instances(
        self, dataset, cols, k, distance_metric, max_iters, n_inits, p=1
    ):
        ...
        kmeans = KMeans(
            n_clusters=k, max_iter=max_iters, n_init=n_inits, random_state=0
        ).fit(temp_dataset)
    ...
    def k_means_over_datasets(
        ...
        return self.k_means_over_instances(
            temp_dataset,
    ...
    def compute_distance_matrix_instances(self, dataset, distance_metric):
        ...
        if not (
            distance_metric
            in [self.manhattan, self.minkowski, self.gower, self.euclidean]
        ):
            ...
            return pd.DataFrame(
                distances,
                index=range(0, len(dataset.index)),
                columns=range(0, len(dataset.index)),
            )
        # Create an empty pandas dataframe for our distance matrix
        distances = pd.DataFrame(
            index=range(0, len(dataset.index)), columns=range(0, len(dataset.index))
        )
        DM = InstanceDistanceMetrics()
        ...
        for i in range(0, len(dataset.index)):
            for j in range(i, len(dataset.index)):
                if distance_metric == self.manhattan:
                    distances.iloc[i, j] = self.manhattan_distance(
                        dataset.iloc[i : i + 1, :], dataset.iloc[j : j + 1, :]
                    )
                elif distance_metric == self.minkowski:
                    distances.iloc[i, j] = self.manhattan_distance(
                        dataset.iloc[i : i + 1, :], dataset.iloc[j : j + 1, :], self.p
                    )
                elif distance_metric == self.gower:
                    distances.iloc[i, j] = self.gowers_similarity(
                        dataset.iloc[i : i + 1, :], dataset.iloc[j : j + 1, :]
                    )
                elif distance_metric == self.euclidean:
                    distances.iloc[i, j] = self.euclidean_distance(
                        dataset.iloc[i : i + 1, :], dataset.iloc[j : j + 1, :]
                    )
    ...
    def k_medoids_over_instances(
        self, dataset, cols, k, distance_metric, max_iters, n_inits=5, p=1
    ):
            ...
            for it in range(0, n_inits):
                ...
                while (n_iter < max_iters) and not (centers == prev_centers):
                    ...
                    for i in range(0, k):
                        ...
                        best_center = (
                            D.loc[points_to_centroid == centers[i]].sum().idxmin(axis=1)
                        )
        ...
        silhouette_per_inst = silhouette_samples(
            temp_dataset, np.array(cluster_assignment)
        )
        dataset["silhouette"] = silhouette_per_inst
    ...
    def compute_distance_matrix_datasets(self, datasets, distance_metric):
        distances = pd.DataFrame(
            index=range(0, len(datasets)), columns=range(0, len(datasets))
        )
    ...
    def k_medoids_over_datasets(
        ...
        if abstraction_method in [self.abstraction_mean, self.abstraction_normal]:
            ...
            return self.k_medoids_over_instances(
                temp_dataset,
                temp_dataset.columns,
                k,
                distance_metric,
...
class HierarchicalClustering:
    ...
    def agglomerative_over_instances(
        self,
        dataset,
        cols,
        max_clusters,
        ...
        if (not use_prev_linkage) or (self.link is None):
            ...
            if distance_metric == df.manhattan:
                self.link = linkage(
                    temp_dataset.values, method=link_function, metric="cityblock"
                )
            else:
                self.link = linkage(
                    temp_dataset.values, method=link_function, metric="euclidean"
                )
    ...
    def agglomerative_over_datasets(
        self,
        datasets,
        cols,
        max_clusters,
        ...
        # And simply apply the instance based algorithm...
        return self.agglomerative_over_instances(
            temp_dataset,
            temp_dataset.columns,
            max_clusters,
            distance_metric,
            use_prev_linkage=use_prev_linkage,
