...
class TemporalClassificationAlgorithms:
    ...
    def create_numerical_single_dataset(self, dataset):
        return copy.deepcopy(
            pd.get_dummies(pd.DataFrame(dataset), prefix="", prefix_sep="")
        )
    ...
    def create_numerical_multiple_dataset(self, train, test):
        ...
        total_dataset = pd.get_dummies(
            pd.DataFrame(total_dataset), prefix="", prefix_sep=""
        )
        new_train = copy.deepcopy(total_dataset.iloc[0 : len(train.index), :])
        new_test = copy.deepcopy(
            total_dataset.iloc[len(train.index) : len(train.index) + len(test.index), :]
        )
    ...
    def predict_values_echo_state_network(
        self, Win, W, Wback, Wout, a, reservoir_size, X, y_true, cols, per_time_step
    ):
        # http://minds.jacobs-university.de/mantas/code
        ...
        for t in range(0, len(X.index)):
            ...
            x = (1 - a) * x + a * np.tanh(
                np.dot(Win, np.vstack(np.insert(u, 0, 1)))
                + np.dot(W, x)
                + np.dot(Wback, np.vstack(y_prev))
            )
    ...
    def generate_parameter_combinations(self, parameter_dict, params):
        ...
        if len(params) == 1:
            ...
            params_without_first_element_combinations = (
                self.generate_parameter_combinations(
                    parameter_dict, params_without_first_element
                )
            )
    ...
    def gridsearch_reservoir_computing(
        self,
        train_X,
        train_y,
        test_X,
        ...
        train_params_X = train_X.iloc[
            0:split_point,
        ]
        test_params_X = train_X.iloc[
            split_point : len(train_X.index),
        ]
        train_params_y = train_y.iloc[
            0:split_point,
        ]
        test_params_y = train_y.iloc[
            split_point : len(train_X.index),
        ]
        ...
        print("-------")
        return (
            best_combination[keys.index("reservoir_size")],
            best_combination[keys.index("a")],
        )
    ...
    def reservoir_computing(
        ...
        if gridsearch:
            reservoir_size, a = self.gridsearch_reservoir_computing(
                train_X,
                train_y,
                test_X,
                test_y,
        ...
        new_train_y, new_test_y, min_y, max_y = self.normalize(
            new_train_y, new_test_y, -0.9, 0.9
        )
        ...
        Win, W, Wback = self.initialize_echo_state_network(
            inputs, outputs, reservoir_size
        )
        ...
        for t in range(0, len(new_train_X.index)):
            ...
            if t >= washout_period:
                X[t - washout_period, :] = np.hstack(
                    np.insert(np.insert(x, 0, u), 0, 1)
                )
        ...
        pred_train_y, pred_train_y_prob = self.predict_values_echo_state_network(
            Win,
            W,
            Wback,
            Wout,
        ...
        pred_test_y, pred_test_y_prob = self.predict_values_echo_state_network(
            Win,
            W,
            Wback,
            Wout,
    ...
    def gridsearch_recurrent_neural_network(
        self,
        train_X,
        train_y,
        test_X,
        ...
        print("-------")
        return (
            best_combination[params.index("n_hidden_neurons")],
            best_combination[params.index("iterations")],
    ...
    def recurrent_neural_network(
        ...
        if gridsearch:
            (
                ...
                iterations,
                outputbias,
            ) = self.gridsearch_recurrent_neural_network(
                train_X,
                train_y,
                test_X,
                test_y,
        ...
        y_train_result = pd.DataFrame(
            Y_train, columns=new_train_y.columns, index=train_y.index
        )
        y_test_result = pd.DataFrame(
            Y_test, columns=new_test_y.columns, index=test_y.index
        )
        ...
        return (
            y_train_result.idxmax(axis=1),
            y_test_result.idxmax(axis=1),
            y_train_result,
            y_test_result,
        )
...
class TemporalRegressionAlgorithms:
    ...
    # on the Pareto front.
    def dynamical_systems_model_nsga_2(
        self,
        train_X,
        train_y,
        test_X,
        ...
        final_pop = ea.evolve(
            ...
            evaluator=evaluator.evaluator_multi_objective,
            pop_size=pop_size,
            maximize=False,
            bounder=None,
        ...
        for f in final_arc:
            ...
            # Predict the results.
            train_fitness, y_train_pred = evaluator.predict(
                f.candidate, training=True, per_time_step=per_time_step
            )
            test_fitness, y_test_pred = evaluator.predict(
                f.candidate, training=False, per_time_step=per_time_step
            )
    ...
    def dynamical_systems_model_ga(
        ...
        final_pop = ea.evolve(
            generator=evaluator.generator,
            evaluator=evaluator.evaluator_single_objective,
            pop_size=pop_size,
            maximize=False,
            bounder=None,
            max_generations=max_generations,
        ...
        best = min(final_pop)
        train_fitness, y_train_pred = evaluator.predict(
            best.candidate, training=True, per_time_step=per_time_step
        )
        test_fitness, y_test_pred = evaluator.predict(
            best.candidate, training=False, per_time_step=per_time_step
        )
    ...
    # observed values.
    # It returns a prediction for the training and test sets that are the result of the best parameter setting.
    def dynamical_systems_model_sa(
        self,
        train_X,
        train_y,
        test_X,
    ...
    def gridsearch_time_series(
        ...
        print("-------")
        return (
            best_combination[keys.index("ar")],
            best_combination[keys.index("ma")],
            best_combination[keys.index("d")],
        )
    ...
    def time_series(
        ...
        if gridsearch:
            ar, ma, d = self.gridsearch_time_series(
                train_X,
                train_y,
                test_X,
                test_y,
        ...
        model_pred = model.predict(
            h=len(train_y.index) - max(ar, ma), oos_data=train_dataset
        )
        ...
        pred_test = pd.DataFrame(
            model.predict(h=len(test_y.index), oos_data=test_dataset).values,
            index=test_y.index,
            columns=[test_y.name],
        )
