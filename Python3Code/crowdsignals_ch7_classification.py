...
features, ordered_features, ordered_scores = fs.forward_selection(
    N_FORWARD_SELECTION, train_X[features_after_chapter_5], train_y, gridsearch=False
)

DataViz.plot_xy(
    x=[range(1, N_FORWARD_SELECTION + 1)],
    y=[ordered_scores],
...
selected_features = [
    ...
    "press_phone_pressure_temp_mean_ws_120",
    "gyr_phone_x_temp_std_ws_120",
    "mag_watch_y_pse",
    "mag_phone_z_max_freq",
    "gyr_watch_y_freq_weighted",
    "gyr_phone_y_freq_1.0_Hz_ws_40",
    "acc_phone_x_freq_1.9_Hz_ws_40",
    "mag_watch_z_freq_0.9_Hz_ws_40",
...
DataViz.plot_xy(
    x=[reg_parameters, reg_parameters],
...
for i in range(0, len(possible_feature_sets)):
    ...
    for repeat in range(0, N_KCV_REPEATS):
        print(
            "Training NeuralNetwork run {} / {} ... ".format(
                repeat, N_KCV_REPEATS, feature_names[i]
            )
        )
        (
            class_train_y,
        ...
        print(
            "Training RandomForest run {} / {} ... ".format(
                repeat, N_KCV_REPEATS, feature_names[i]
            )
        )
        ...
        print(
            "Training SVM run {} / {}, featureset: {}... ".format(
                repeat, N_KCV_REPEATS, feature_names[i]
            )
        )
    ...
    print(
        "Training Nearest Neighbor run 1 / 1, featureset {}:".format(feature_names[i])
    )
    (
        class_train_y,
        class_test_y,
    ...
    (
        ...
        class_train_prob_y,
        class_test_prob_y,
    ) = learner.naive_bayes(selected_train_X, train_y, selected_test_X)
...
DataViz.plot_performances_classification(
    ["NN", "RF", "SVM", "KNN", "DT", "NB"], feature_names, scores_over_all_algs
)
