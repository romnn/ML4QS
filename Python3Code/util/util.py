...
def print_latex_table_statistics_two_datasets(dataset1, dataset2):
    ...
    for col in dataset1.columns:
        print(col, "& ", end="")
        print_table_cell(
            (float((dataset1_length - dataset1[col].count())) / dataset1_length) * 100,
    ...
    for c in input_cols:
        ...
        for cluster in clusters:
            print(
                " & ",
                "{0:.2f}".format(
                    dataset.loc[dataset[cluster_col] == cluster, c].mean()
                ),
                end="",
            )
        print("\\\\")
        ...
        for cluster in clusters:
            print(
                " & ",
                "{0:.2f}".format(dataset.loc[dataset[cluster_col] == cluster, c].std()),
                end="",
            )
        print("\\\\")
    ...
    for l in label_cols:
        ...
        for cluster in clusters:
            print(
                ...
                "{0:.2f}".format(
                    (
                        float(dataset.loc[dataset[cluster_col] == cluster, l].sum())
                        / len(dataset[dataset[l] == 1].index)
                        * 100
                    )
                ),
                "\%",
